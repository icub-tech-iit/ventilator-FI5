...
static int s_hal_trace_puts(const char* str);
...
// it implements the C puts() but sends output to the trace port. 
// see http://www.cplusplus.com/reference/cstdio/puts/
static int s_hal_trace_puts(const char* str) 
{    
...
static void s_init_pushbutton(void)
{
    ...
    s_hal_trace_puts("pushbutton initted");
}
...
static void s_init_lcd(void)
{
    ...
    BSP_LCD_DisplayStringAtLine(8, (uint8_t *)"now led blinks @ 1.0 Hz");

    s_hal_trace_puts("lcd initted");
}
...
static void s_tick00(void)
{
    ...
    button_state_t state = static_cast<button_state_t>(BSP_PB_GetState(BUTTON_KEY));
    
    if(button_state_pressed == state)
    {
        s_hal_trace_puts("button pressed");

        BSP_LCD_DisplayStringAtLine(7, (uint8_t *)"button pressed");

        delay = (500 == delay) ? (1000) : (500);  

        BSP_LCD_ClearStringLine(8);            
        BSP_LCD_DisplayStringAtLine(8, (uint8_t *)((500==delay)?("now led blinks @ 1.0 Hz"):("now led blinks @ 0.5 Hz")));
    } 
    else
    {
