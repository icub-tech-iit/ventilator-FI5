...
typedef enum
{
    button_state_pressed = GPIO_PIN_SET,
    button_state_unpressed = GPIO_PIN_RESET
} button_state_t;
...
static void s_init_pushbutton(void);
static void s_init_lcd(void);
static void s_test_qspi(void);
...
vnt::core::Time ledperiod = vnt::core::time1second;

static const unsigned char s_stlogo[9174] =
{
...
static void s_init_led(void)
{
    ...
    theleds.get(vnt::bsp::LED::one).pulse(ledperiod); 
}
...
static void s_init_pushbutton(void)
{
    ...
    vnt::bsp::button::init(vnt::bsp::BTN::three);
    vnt::bsp::trace::puts("pushbutton initted");
}

static void s_test_qspi(void)
{
    volatile uint8_t r = BSP_QSPI_Init();
    r = r;    
    ...
    uint8_t back[32] = {0};
    r = BSP_QSPI_Read(back, 0, sizeof(back));
    if(hello[0] != back[0])
    {
        r = BSP_QSPI_Erase_Block(0);
        r = BSP_QSPI_Write(hello, 0, sizeof(hello));
        r = r;
    }
...
static void s_init_lcd(void)
{
    ...
    // this stm function is NOT properly documented. 
    // examples show that the second argument is ... just a RAM address
    // but i could not understand if the memory is reserved by anybody and how much of it
    // TBD: investigate better. it is a potential danger.
    BSP_LCD_LayerDefaultInit(LTDC_ACTIVE_LAYER, LCD_FRAME_BUFFER);

    
    static const uint8_t welcome[] = "hello world";

    // the code that follows is taken from a stm example
    ...
    BSP_LCD_DisplayStringAt(0, 35, (uint8_t *)welcome, CENTER_MODE);

    BSP_LCD_DrawRect(10, 20, 20, 30);

    BSP_LCD_FillRect(40, 20, 20, 30);

    BSP_LCD_DrawBitmap((BSP_LCD_GetXSize() - 80 - 10), 20, (uint8_t *)s_stlogo);
    
    BSP_LCD_SetFont(&Font16);
    
    BSP_LCD_DisplayStringAtLine(5, (uint8_t *)"a LED is blinking.");
    BSP_LCD_DisplayStringAtLine(6, (uint8_t *)"press button to change rate");
    BSP_LCD_DisplayStringAtLine(9, (uint8_t *)"now led blinks @ 1.0 Hz");

    vnt::bsp::trace::puts("lcd initted");
}

// it is just one kind of tick
static void s_tick00(void)
{
    static const uint8_t str1hz[] = "now led blinks @ 1.0 Hz";
    static const uint8_t str2hz[] = "now led blinks @ 2.0 Hz";
    

    if(true == vnt::bsp::button::pressed(vnt::bsp::BTN::three))
    {
        vnt::bsp::trace::puts("button pressed");

        BSP_LCD_DisplayStringAtLine(7, (uint8_t *)"button pressed");

        ledperiod = (500*vnt::core::time1millisec == ledperiod) ? (vnt::core::time1second) : (500*vnt::core::time1millisec);  
        vnt::app::theLEDmanager::getInstance().get(vnt::bsp::LED::one).pulse(ledperiod);         

        BSP_LCD_ClearStringLine(9);            
        //BSP_LCD_DisplayStringAtLine(9, (uint8_t *)((vnt::core::time1second==ledperiod)?(str1hz):(str2hz)));
    } 
    else
    {
...
volatile vnt::core::Time tt = vnt::core::now();
    
    HAL_Delay(delay);
    
    volatile vnt::core::Time tt1 = vnt::core::now();
    
    stm32f7_if2cpp_tick();
    
    vnt::core::Time delta = vnt::core::now() - tt1;
    delta =  delta;

    char txt[32] = {0};
    snprintf(txt, sizeof(txt), "duration = %d ms", (uint32_t)(delta/1000));
    vnt::core::TimeFormatter tf(delta);
    vnt::bsp::trace::puts("duration = " + tf.to_string());
