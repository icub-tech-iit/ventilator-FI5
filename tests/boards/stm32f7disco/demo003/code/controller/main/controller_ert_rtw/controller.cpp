...
// marco.accame: 
#warning this is the single precision code version provided by ugo.pattacini on 10 apr 2020
...
void controllerModelClass::step()
{
  ...
  if (controller_U.enable) {
    ...
    if (controller_P.Constant1_Value_p) {
      ...
      rtb_Switch2 = (controller_P.Controller_VC_P * rtb_Switch2 +
                     controller_B.Integrator) + controller_B.DataStoreRead;

      // Saturate: '<S4>/Saturation'
      if (rtb_Switch2 > controller_P.Saturation_UpperSat_b) {
        controller_B.Saturation = controller_P.Saturation_UpperSat_b;
      } else if (rtb_Switch2 < controller_P.Saturation_LowerSat_f) {
        controller_B.Saturation = controller_P.Saturation_LowerSat_f;
      } else {
        controller_B.Saturation = rtb_Switch2;
      }
    ...
    rtb_Switch2 = controller_DW.obj.FilterObj->cSFunObject.P1_RTP1COEFF[0] *
      controller_B.cmH20;
    rtb_Switch2 += controller_DW.obj.FilterObj->cSFunObject.P1_RTP1COEFF[1] *
      controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[0];
    rtb_Switch2 += controller_DW.obj.FilterObj->cSFunObject.P1_RTP1COEFF[2] *
      controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[1];
    rtb_Switch2 -= controller_DW.obj.FilterObj->cSFunObject.P2_RTP2COEFF[0] *
      controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[0];
    rtb_Switch2 -= controller_DW.obj.FilterObj->cSFunObject.P2_RTP2COEFF[1] *
      controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[1];
    controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[1] =
      controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[0];
    controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[0] =
    ...
    controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[1] =
      controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[0];
    controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[0] = rtb_Switch2;
    controller_B.HSCLP = rtb_Switch2;
    ...
    if (controller_P.Constant2_Value) {
      ...
      // Sum: '<S3>/Sum1'
      rtb_Switch2 = rtb_InputShaping - controller_B.HSCLP;

      // Relay: '<S3>/Relay'
      controller_DW.Relay_Mode = ((rtb_Switch2 >= controller_P.Relay_OnVal) ||
        ((!(rtb_Switch2 <= controller_P.Relay_OffVal)) &&
         controller_DW.Relay_Mode));
      if (controller_DW.Relay_Mode) {
        Integrator_m_tmp = controller_P.Relay_YOn;
      } else {
        Integrator_m_tmp = controller_P.Relay_YOff;
      }

      // End of Relay: '<S3>/Relay'

      // Switch: '<S3>/Switch2'
      if (Integrator_m_tmp > controller_P.Switch2_Threshold) {
        Integrator_m_tmp = controller_P.Controller_PC_I_up;
      } else {
        Integrator_m_tmp = controller_P.Controller_PC_I_dw;
      }
      ...
      Integrator_m_tmp = controller_P.Integrator_gainval * (rtb_Switch2 *
        Integrator_m_tmp);
      ...
      if (static_cast<real32_T>(rtb_PulseGenerator) >
        ...
        rtb_Switch2 = (rtb_Switch2 * controller_P.Controller_PC_P +
                       controller_B.Integrator_m) +
          controller_P.Controller_PC_FFW;

        // Saturate: '<S3>/Saturation'
        if (rtb_Switch2 > controller_P.Saturation_UpperSat) {
          controller_B.Switch_n = controller_P.Saturation_UpperSat;
        } else if (rtb_Switch2 < controller_P.Saturation_LowerSat) {
          controller_B.Switch_n = controller_P.Saturation_LowerSat;
        } else {
          controller_B.Switch_n = rtb_Switch2;
        }
...
{
    controller_U.enable =  true; // marco.accame: enabled it
  {
    ...
    iobj_0->cSFunObject.P1_RTP1COEFF[0] = 0.192611083F;
    iobj_0->cSFunObject.P1_RTP1COEFF[1] = 0.278119922F;
    iobj_0->cSFunObject.P1_RTP1COEFF[2] = 0.192611083F;
    iobj_0->cSFunObject.P2_RTP2COEFF[0] = -0.616777062F;
    iobj_0->cSFunObject.P2_RTP2COEFF[1] = 0.287800312F;
