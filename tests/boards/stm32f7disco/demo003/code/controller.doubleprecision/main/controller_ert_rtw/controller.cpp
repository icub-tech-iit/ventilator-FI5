...
void controllerModelClass::step()
{
  ...
  if (controller_U.enable) {
    ...
    controller_Y.OV_cmd = controller_P.Constant_Value_a - rtb_PulseGenerator;
    ...
    if (controller_P.Constant1_Value_p) {
      ...
      // Gain: '<S4>/Volumetric Flow rate'
      controller_B.VolumetricFlowrate = 0.65 / (1.0 /
        controller_P.Controller_respiratory_rate *
        controller_P.Controller_IE_ratio / (controller_P.Controller_IE_ratio +
        1.0)) * rtb_PulseGenerator;
      ...
      if (controller_B.VolumetricFlowrate > 0.0) {
      ...
      // Sum: '<S4>/Sum1'
      rtb_Switch2 = controller_B.VolumetricFlowrate - controller_B.Gain1;
      ...
      rtb_InputShaping = controller_P.Integrator_gainval_m *
        (controller_P.Controller_VC_I * rtb_Switch2);
      controller_B.Integrator = rtb_InputShaping +
        controller_DW.Integrator_DSTATE;
      ...
      rtb_Sum1 = (controller_P.Controller_VC_P * rtb_Switch2 +
      ...
      controller_DW.Integrator_DSTATE = rtb_InputShaping +
        controller_B.Integrator;
      if (controller_B.VolumetricFlowrate > 0.0) {
        controller_DW.Integrator_PrevResetState = 1;
      } else if (controller_B.VolumetricFlowrate < 0.0) {
        controller_DW.Integrator_PrevResetState = -1;
      } else if (controller_B.VolumetricFlowrate == 0.0) {
    ...
    controller_B.cmH20 = (controller_P.Gain_Gain_p *
                          controller_DW.UnitDelay1_DSTATE +
                          controller_P.Bias_Bias_e) * controller_U.params[1] *
      controller_P.Gain1_Gain_c * controller_P.cmH20_Gain;
    ...
    rtb_Sum1 = controller_DW.obj.FilterObj->cSFunObject.P1_RTP1COEFF[0] *
      controller_B.cmH20;
    rtb_Sum1 += controller_DW.obj.FilterObj->cSFunObject.P1_RTP1COEFF[1] *
      controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[0];
    rtb_Sum1 += controller_DW.obj.FilterObj->cSFunObject.P1_RTP1COEFF[2] *
      controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[1];
    rtb_Sum1 -= controller_DW.obj.FilterObj->cSFunObject.P2_RTP2COEFF[0] *
      controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[0];
    rtb_Sum1 -= controller_DW.obj.FilterObj->cSFunObject.P2_RTP2COEFF[1] *
      controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[1];
    controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[1] =
      controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[0];
    controller_DW.obj.FilterObj->cSFunObject.W0_ZERO_STATES[0] =
    ...
    controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[1] =
      controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[0];
    controller_DW.obj.FilterObj->cSFunObject.W1_POLE_STATES[0] = rtb_Sum1;
    controller_B.HSCLP = rtb_Sum1;
    ...
    if (controller_P.Constant2_Value) {
      ...
      controller_DW.InputShaping_tmp = (((controller_P.Controller_PIP *
        rtb_PulseGenerator - controller_P.InputShaping_DenCoef[1] *
      ...
      rtb_InputShaping = ((controller_P.InputShaping_NumCoef[0] *
      ...
      // Sum: '<S3>/Sum1'
      rtb_Sum1 = rtb_InputShaping - controller_B.HSCLP;

      // Relay: '<S3>/Relay'
      rtb_Switch2 = std::abs(controller_P.Controller_PC_I_e_thres_sched);
      controller_DW.Relay_Mode = ((rtb_Sum1 >= rtb_Switch2) || ((!(rtb_Sum1 <=
        -rtb_Switch2)) && controller_DW.Relay_Mode));
      if (controller_DW.Relay_Mode) {
        rtb_Switch2 = controller_P.Relay_YOn;
      } else {
        rtb_Switch2 = controller_P.Relay_YOff;
      }
      ...
      if (rtb_Switch2 > controller_P.Switch2_Threshold) {
        rtb_Switch2 = controller_P.Controller_PC_I_up;
      } else {
        rtb_Switch2 = controller_P.Controller_PC_I_dw;
      }
      ...
      rtb_Switch2 = controller_P.Integrator_gainval * (rtb_Sum1 * rtb_Switch2);
      controller_B.Integrator_m = rtb_Switch2 +
        controller_DW.Integrator_DSTATE_p;
      ...
      if (rtb_PulseGenerator > controller_P.Switch_Threshold) {
        // Sum: '<S3>/Sum' incorporates:
        //   Product: '<S43>/PProd Out'
        //   Sum: '<S47>/Sum'

        rtb_Sum1 = (rtb_Sum1 * controller_P.Controller_PC_P +
      ...
      // Switch: '<S3>/Switch1'
      if (rtb_PulseGenerator > controller_P.Switch1_Threshold) {
        controller_B.Switch1 = rtb_InputShaping;
      } else {
      ...
      controller_DW.Integrator_DSTATE_p = rtb_Switch2 +
    ...
    if (controller_DW.Controller_MODE) {
      ...
      controller_B.cmH20 = controller_P.signals_Y0[1];
  ...
  controller_Y.signals[1] = controller_B.cmH20;
...
{
    controller_U.enable = true; // marco.accame
  {
    ...
    iobj_0->cSFunObject.P1_RTP1COEFF[0] = 0.19261109082499325;
    iobj_0->cSFunObject.P1_RTP1COEFF[1] = 0.27811992575641481;
    iobj_0->cSFunObject.P1_RTP1COEFF[2] = 0.19261109082499323;
    iobj_0->cSFunObject.P2_RTP2COEFF[0] = -0.6167770586229675;
    iobj_0->cSFunObject.P2_RTP2COEFF[1] = 0.28780030562074577;
